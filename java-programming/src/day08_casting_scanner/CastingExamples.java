package day08_casting_scanner;

import java.sql.SQLOutput;

public class CastingExamples {
    public static void main (String [] args) {
        // byte - short - int - long
        byte num1 = 100;
        short num2 = (short)num1;
        int num3 = num2;
        int num4 = 1000;
        long num5 = num4;
        // float - double
        float num6 = 124.4F;
        double num7 = num6;

        int num8 = 3456;
        double num9 = num8;
        System.out.println(num8); //3456
        System.out.println("num9 = " + num9);

        int num10 = 22;
        byte num11 = (byte)num10;
        short num12 = (short)num11;
        System.out.println("num10 =" + num10);
        System.out.println("num11 =" + num11);
        System.out.println("num12 =" + num12);

        double num13 =55.3;
        float num14 =(float)num13;
        System.out.println("num13 =" + num13);
        System.out.println("num14 =" + num14);

        double num15 = 1234.5;
        int num16 =(int)num15;
        System.out.println("num15 =" + num15);
        System.out.println("num16 =" + num16);

        int num17 =300;
        double num18 =(byte)num17;
        System.out.println("num17 =" + num17);
        System.out.println("num18 =" + num18);

        char letter = 'G';
        int numLetter = letter;
        System.out.println("letter = " + letter);
        System.out.println("numLetter = " + numLetter);

        char ch = 'J';
        int chNum = ch;
        System.out.println(chNum);

        byte gulya = 100;
        short ariella = gulya;

        short gulina = 200;
        int aigul = gulina;

        int jika = 1000;
        long mirba = jika;

        long amir = 10000l;
        int alihan = (int)amir;
        System.out.println(amir);
        System.out.println(alihan);

        double aa = 988909876895768908765434.8;
        int bb = (int) aa;
        System.out.println(aa); //55.4
        System.out.println(bb); //55

        char table = '$';
        int chair = table;
        System.out.println("table = " + table);
        System.out.println("chair = " + chair);

        double spoon = 78658989.87458;
        short cup = (short)spoon;
        System.out.println("spoon = " + spoon);
        System.out.println("cup = " + cup);

        int c = 5;
        byte n = (byte)c;
        System.out.println("n = " + n);

    }
}
